...
for i in ${!nthreads[@]}; do
    for j in ${!L[@]}; do
        for k in ${!space_grid[@]}; do
            echo "./solver ${L[$j]} ${L[$j]} ${L[$j]} 0.05 ${space_grid[$k]} 100 ${nthreads[$i]}"
            echo "./solver ${L[$j]} ${L[$j]} ${L[$j]} 0.05 ${space_grid[$k]} 100 ${nthreads[$i]}" >> launches.txt
            ./solver ${L[$j]} ${L[$j]} ${L[$j]} 0.05 ${space_grid[$k]} 100 ${nthreads[$i]}
        done
    done
done