...
struct Uapproximated {
    ...
    double *buf_xy_u = nullptr, *buf_xz_u = nullptr, *buf_yz_u = nullptr;
    double *buf_xy_d = nullptr, *buf_xz_d = nullptr, *buf_yz_d = nullptr;
    ...
    explicit Uapproximated(int N, int K, const Point& L, double T, int ntasks, int task_id): N(N), K(K), ntasks(ntasks), task_id(task_id) {
        ...
        buf_xy_u = new double[sizes.by_x * sizes.by_y];
        buf_xz_u = new double[sizes.by_x * sizes.by_z];
        buf_yz_u = new double[sizes.by_y * sizes.by_z];
        buf_xy_d = new double[sizes.by_x * sizes.by_y];
        buf_xz_d = new double[sizes.by_x * sizes.by_z];
        buf_yz_d = new double[sizes.by_y * sizes.by_z];
    ...
    void SendRecvLowerBuffers(double*** grid, std::vector<MPI_Request>& requests) {
        // Copy buffers
        #pragma omp parallel for collapse(2)
        for (int i = 0; i < sizes.by_x; ++i) {
            for (int j = 0; j < sizes.by_y; ++j) {
                buf_xy_d[i * sizes.by_y + j] = grid[i][j][0];
            }
        }
        #pragma omp parallel for collapse(2)
        for (int i = 0; i < sizes.by_x; ++i) {
            for (int j = 0; j < sizes.by_z; ++j) {
                buf_xz_d[i * sizes.by_z + j] = grid[i][0][j];
            }
        }
        #pragma omp parallel for collapse(2)
        for (int i = 0; i < sizes.by_y; ++i) {
            for (int j = 0; j < sizes.by_z; ++j) {
                buf_yz_d[i * sizes.by_z + j] = grid[0][i][j];
            }
        }
        ...
        MPI_Isend(buf_xy_d, sizes.by_x * sizes.by_y, MPI_DOUBLE, dest, kBufTagXy, MPI_COMM_WORLD, &(requests[0]));
        ...
        MPI_Isend(buf_xz_d, sizes.by_x * sizes.by_z, MPI_DOUBLE, dest, kBufTagXz, MPI_COMM_WORLD, &(requests[1]));
        ...
        MPI_Isend(buf_yz_d, sizes.by_y * sizes.by_z, MPI_DOUBLE, dest, kBufTagYz, MPI_COMM_WORLD, &(requests[2]));
        ...
        MPI_Irecv(xy_boarder_u, sizes.by_x * sizes.by_y, MPI_DOUBLE, src, kBufTagXy, MPI_COMM_WORLD, &(requests[3]));
        ...
        MPI_Irecv(xz_boarder_u, sizes.by_x * sizes.by_z, MPI_DOUBLE, src, kBufTagXz, MPI_COMM_WORLD, &(requests[4]));
        ...
        MPI_Irecv(yz_boarder_u, sizes.by_z * sizes.by_y, MPI_DOUBLE, src, kBufTagYz, MPI_COMM_WORLD, &(requests[5]));
    }

    void SendRecvUpperBuffers(double*** grid, std::vector<MPI_Request>& requests) {
        // Copy buffers
        #pragma omp parallel for collapse(2)
        for (int i = 0; i < sizes.by_x; ++i) {
            for (int j = 0; j < sizes.by_y; ++j) {
                buf_xy_u[i * sizes.by_y + j] = grid[i][j][sizes.by_z - 1];
            }
        }
        #pragma omp parallel for collapse(2)
        for (int i = 0; i < sizes.by_x; ++i) {
            for (int j = 0; j < sizes.by_z; ++j) {
                buf_xz_u[i* sizes.by_z + j] = grid[i][sizes.by_y - 1][j];
            }
        }
        #pragma omp parallel for collapse(2)
        for (int i = 0; i < sizes.by_y; ++i) {
            for (int j = 0; j < sizes.by_z; ++j) {
                buf_yz_u[i * sizes.by_z + j] = grid[sizes.by_x - 1][i][j];
            }
        }
        ...
        dest = receiving_task_id_z + proc_pos.by_y * g.by_z + proc_pos.by_x * g.by_y * g.by_z;
        MPI_Isend(buf_xy_u, sizes.by_x * sizes.by_y, MPI_DOUBLE, dest, kBufTagXy, MPI_COMM_WORLD, &(requests[6]));
        int receiving_task_id_y = proc_pos.by_y + 1;
        if (receiving_task_id_y >= g.by_y) receiving_task_id_y = 0;
        dest = proc_pos.by_z + receiving_task_id_y * g.by_z + proc_pos.by_x * g.by_y * g.by_z;
        MPI_Isend(buf_xz_u, sizes.by_x * sizes.by_z, MPI_DOUBLE, dest, kBufTagXz, MPI_COMM_WORLD, &(requests[7]));
        int receiving_task_id_x = proc_pos.by_x + 1;
        if (receiving_task_id_x >= g.by_x) receiving_task_id_x = 0;
        dest = proc_pos.by_z + proc_pos.by_y * g.by_z + receiving_task_id_x * g.by_y * g.by_z;
        MPI_Isend(buf_yz_u, sizes.by_y * sizes.by_z, MPI_DOUBLE, dest, kBufTagYz, MPI_COMM_WORLD, &(requests[8]));
        ...
        src = sender_task_id_z + proc_pos.by_y * g.by_z + proc_pos.by_x * g.by_y * g.by_z;
        MPI_Irecv(xy_boarder_d, sizes.by_x * sizes.by_y, MPI_DOUBLE, src, kBufTagXy, MPI_COMM_WORLD, &(requests[9]));
        int sender_task_id_y = proc_pos.by_y - 1;
        if (sender_task_id_y < 0) sender_task_id_y = g.by_y - 1;
        src = proc_pos.by_z + sender_task_id_y * g.by_z + proc_pos.by_x * g.by_y * g.by_z;
        MPI_Irecv(xz_boarder_d, sizes.by_x * sizes.by_z, MPI_DOUBLE, src, kBufTagXz, MPI_COMM_WORLD, &(requests[10]));
        int sender_task_id_x = proc_pos.by_x - 1;
        if (sender_task_id_x < 0) sender_task_id_x = g.by_x - 1;
        src = proc_pos.by_z + proc_pos.by_y * g.by_z + sender_task_id_x * g.by_y * g.by_z;
        MPI_Irecv(yz_boarder_d, sizes.by_z * sizes.by_y, MPI_DOUBLE, src, kBufTagYz, MPI_COMM_WORLD, &(requests[11]));
    }
    ...
    void SendBuffers(bool is_grid_n0) {
        ...
        std::vector<MPI_Request> requests(12);
        SendRecvLowerBuffers(grid, requests);
        SendRecvUpperBuffers(grid, requests);
...
void DumpMetaInfo(const std::string& path_prefix, const double error, double memory_time, double diffur_time, int nthreads, int ntasks) {
...
int main(int argc, char** argv) {
    ...
    if (!task_id) {
        std::cout << "Number of threads: " << Nthreads << ", max threads: " << omp_get_max_threads() <<  std::endl;
        std::cout << "Number of processes: " << ntasks << std::endl;
    }
    const std::string kDumpPrefixFolder{
        std::string("results/MPI_") + argv[1] + "_" + argv[2] + "_" + argv[3] + "_" + argv[4] + "_" + argv[5] + "_" + argv[6] + "_" + argv[7] + "_" + std::to_string(ntasks)
    };
    ...
    t_diffur = std::chrono::high_resolution_clock::now();       // Start time counting
    solution.FillZerothLayer(u);
    solution.SendBuffers(true);
    solution.FillFirstLayer(a2, u);
    // Count the rest steps
    for (int t = 2; t < K; ++t) {
        solution.SendBuffers(false);
        solution.FillNextLayer(u, a2);
        if (t != K - 1) {                                       // To prevent grid layer move for error count
            solution.MoveGridLayer();
        }
    }
    diffur_time = GetTimeDiff(t_diffur);                        // End time counting
 
    double max_error = solution.CountMaxError(u, K - 1);
    double max_t_diffur, max_global_error;
    MPI_Allreduce(&max_error, &max_global_error, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
    MPI_Allreduce(&diffur_time, &max_t_diffur, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
    if (!task_id) {
        DumpMetaInfo(kDumpPrefixFolder, max_global_error, memory_time, max_t_diffur, Nthreads, ntasks);
    }
    MPI_Finalize();
    return 0;
}